...
int main(int argc, char *argv[])
{
   ...
   while (fgets(line,MAX_LINE,stdin) != NULL) {
      ...
      else if (sscanf(line, "dec %c", &b1) == 1) {
         if ((i1 = argIndex(b1)) < 0) continue;
         showDecimal(bits[i1]); printf("\n");
      }
      else if (sscanf(line, "hex %c", &b1) == 1) {
         if ((i1 = argIndex(b1)) < 0) continue;
         showHex(bits[i1]); printf("\n");
      }
      else if (sscanf(line, "oct %c", &b1) == 1) {
         if ((i1 = argIndex(b1)) < 0) continue;
         showOct(bits[i1]); printf("\n");
      }
      else {
